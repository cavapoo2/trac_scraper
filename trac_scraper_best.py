...
async def scrape_trac_ticket(url: str, output_dir: str = "ticket_data", headless: bool = True):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # IMPORTANT: Exclude timeline links - we only want attachment file links
            attachment_selectors = [
                "#attachments dt a:not([href*='timeline'])",  # Exclude timeline links
                "#attachments dt a[href*='attachment']",      # Only attachment links
                "#attachments a[title]:not([href*='timeline'])"  # Has title, not timeline
            ]
            ...
            # Filter out any remaining timeline or non-attachment links
            filtered_attachments = []
            for attachment in attachments:
                href = await attachment.get_attribute("href")
                
                # Skip if it's a timeline link or doesn't look like an attachment
                if href and (
                    'timeline' in href or 
                    'query' in href or
                    'search' in href or
                    not ('attachment' in href or 'raw-attachment' in href)
                ):
                    print(f"  Skipping non-attachment link: {href}")
                    continue
                
                filtered_attachments.append(attachment)
            
            print(f"  Filtered to {len(filtered_attachments)} actual attachment links")
